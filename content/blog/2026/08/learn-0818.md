...
これだったら、独自のヘルパー関数を用意しても `./src/helper.py`ならば他のsrcのpyファイルと相対的に同じディレクトリなので、`from utils import ...`で呼び出せるので、責務の分離は明確にできますね。
